  ...
  return (
    <form className='min-h-[80vh] flex items-center'>
        <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[360px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
          ...
          <button className='bg-sky-700 text-white w-full py-2 rounded-md text-base ' type="submit" onClick={onSubmitHandler}>{state === 'Sign Up' ? "Create Account":'Login'}</button>
          {
            state === 'Sign Up'
            ?<p>Already have an account? <span onClick={()=> setState('Login')}  className='text-sky-700 underline cursor-pointer'>Login here</span></p> 
            :<p>Create an new account? <span onClick={()=> setState('Sign Up')}  className='text-sky-700 underline cursor-pointer'>click here</span></p>
          }
        </div>
    </form>
  )
